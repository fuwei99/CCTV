...
func main() {
	...
	// Read env var name in uppercase: DEPLOY or fallback to MODE
	deployEnv := strings.ToLower(strings.TrimSpace(os.Getenv("DEPLOY")))
	if deployEnv == "" {
		deployEnv = strings.ToLower(strings.TrimSpace(os.Getenv("MODE")))
	}
	if deployEnv == "cloud" {
		isCloudDeploy = true
	}
